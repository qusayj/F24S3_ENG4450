...
public class Multiplication {
    ...
    public static final long multiplication(int a, int b) {
        // Fix to correctly handle multiplication with negative numbers
        return (long) a * b; // Directly returns the correct result for both positive and negative inputs
    }

    public static final long multiplyUsingLoop(int a, int b) {
        int absA = Math.abs(a);
        int absB = Math.abs(b); // Handle absolute value for both numbers
        long result = 0; // Initialize result to 0
        for (int i = 0; i < absB; i++) { // Iterate absB times
            result += absA;
        }
        return (a > 0 && b > 0 || a < 0 && b < 0) ? result : -result; // Determine the sign of the result
    }

    public static final long multiplyUsingRecursion(int a, int b) {
        int absA = Math.abs(a);
        int absB = Math.abs(b);
        if (absB == 0) return 0; // Base case: multiplication by 0
        if (absB == 1) return absA; // Base case: multiplication by 1
        long result = absA + multiplyUsingRecursion(absA, absB - 1); // Recursive step
        return (a > 0 && b > 0 || a < 0 && b < 0) ? result : -result; // Determine the sign of the result
    }
